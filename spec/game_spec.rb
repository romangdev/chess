...
describe Game do
  ...
  describe '#verify_location_piece' do
    ...
    let(:player_white) do
      double('player_white', pieces: [white_piece.piece_symbol, " \u2656 ", " \u2658 ",
                                      " \u2657 ", " \u2655 ", " \u2654 "])
    end
    let(:player_black) do
      double('player_black', pieces: [black_piece.piece_symbol, " \u265c ", " \u265e ",
                                      " \u265d ", " \u265b ", " \u265a "])
    end

    context 'when location holds a player white piece' do
      it 'returns true' do
        arr = [['check', white_piece], %w[check check]]
        result = game.verify_location_piece(player_white.pieces, [0, 1], arr)
        expect(result).to be true
      end

      it 'returns true' do
        arr = [['check', ' check '], %w[check check], ['check', 'check', white_piece]]
    ...
    context 'when location does not hold a player white piece' do
      it 'returns false' do
        arr = [%w[check check], %w[check check]]
        result = game.verify_location_piece(player_white.pieces, [0, 1], arr)
        expect(result).to be false
      end

      it 'returns false' do
        arr = [['check', ' check '], %w[check check], %w[check check nil]]
        result = game.verify_location_piece(player_white.pieces, [2, 2], arr)
        expect(result).to be false
      end
    end
    ...
    context 'when location holds a player black piece' do
      it 'returns true' do
        arr = [[black_piece, 'nil '], %w[check check]]
        result = game.verify_location_piece(player_black.pieces, [0, 0], arr)
        expect(result).to be true
      end

      it 'returns true' do
        arr = [['check', ' check '], %w[check check], [black_piece, 'check', ' nil']]
