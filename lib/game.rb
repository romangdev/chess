...
class Game
  ...
  def get_player_location(player_color)
    ...
    until flag == true
      ...
      return player_choice if %w[draw d].include?(player_choice)
  ...
  def handle_ask_for_draw(player_choice, player_white, player_black, _done)
    if %w[draw d].include?(player_choice)
      white_answer = verify_draw(player_white.player_color)
      black_answer = verify_draw(player_black.player_color)

      if %w[y yes].include?(white_answer) && %w[y yes].include?(black_answer)
  ...
  def verify_location_piece(player_pieces, chosen_location, board_array)
    player_pieces.each do |piece|
      ...
      elsif board_array[chosen_location[0]][chosen_location[1]].piece_symbol == piece
        return true
      end
    end
  ...
  def handle_confirm_choice(confirm_choice)
    return true if %w[y yes].include?(confirm_choice)
  ...
  def convert_player_column(player_choice)
    COLUMN_MATCH.each do |key, value|
      return value if player_choice[0] == key
    end
  end
