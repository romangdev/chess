...
class Pawn
  ...
  def generate_moves(current_location, chess_board, pawn_color)
    ...
    if pawn_color == WHITE_PAWN
      white_one_square_move(chess_board, current_location, moves)
    else
      black_one_square_move(chess_board, current_location, moves)
    end
  end
  ...
  def check_first_move(moves, current_location, pawn_color, chess_board)
    ...
    elsif current_location[0] == 6 && chess_board[current_location[0] - 2][current_location[1]] == '   ' &&
          chess_board[current_location[0] - 1][current_location[1]] == '   '
      moves << [current_location[0] - 2, current_location[1]]
      self.first_move_made = true
  ...
  def handle_diagonals(right_diagonal, left_diagonal, moves, current_location)
    if piece_symbol == WHITE_PAWN
      handle_white_diagonals(right_diagonal, left_diagonal, current_location, moves)
    else
      handle_black_diagonals(right_diagonal, left_diagonal, current_location, moves)
    end
  end
  ...
  def white_check_diagonal(diagonal)
    if !diagonal.nil? && (diagonal != '   ' && (diagonal.piece_symbol == BLACK_PAWN ||
  ...
  def black_check_diagonal(diagonal)
    if !diagonal.nil? && (diagonal != '   ' && (diagonal.piece_symbol == WHITE_PAWN ||
